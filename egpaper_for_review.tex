...
%%%%%%%%% ABSTRACT
\begin{abstract}
Previous work all focus on noise modeling, which limit their application. However, in this paper, we first propose a framework for real image denoising.  This framework doesn't need noise modeling and estimation. This framework is learning based. A new Double Semi-Couple Dictionary Learing model is proposed to learn the dictionary as well as the mapping function between real noisy images and latent clean natural images. 
\end{abstract}

%%%%%%%%% BODY TEXT
\section{Introduction}

Image denoising is an important problem in computer vision and image processing. High-quality images denoised from noisy ones is also of great importance for high-level vision tasks such as pattern recognition. Over the last several decades, there are many image denoising methods. Most existing methods are designed to deal with specific types of noise, e.g., Gaussian noise, mixed Gaussian and impulse noise, etc. Gaussian noise removal is a fundamental problem which has received intensive research interests with many representative work \cite{rudin1992nonlinear,nlm,foe,ksvd,bm3d,lssc,epll,pgpd,wnnm,csf,chen2015learning,burger2012image}. Image denoising for Gaussian noise is not only an independent task but also can be incorporated into other tasks, e.g., the removal of mixed Gaussian and impulse noise. Despite of the success of these methods, they are designed sepcially for Gaussian noise or other specific types of noise. However, in practice, real noisy images are almost everywhere, in which the noise model is unknown and complex. The noise distribution in real images is more complex than Gaussian or mixed Gaussian and impulse noise, and is usually unknown for existing methods. This makes image denoising still a challenging problem.

In order to deal with real noise in real world images, many methods have been proposed for real or real image denoising problem \cite{fullyblind,rabie2005robust,Liu2008,almapg,noiseclinic,Zhu_2016_CVPR,crosschannel2016}. Early methods \cite{fullyblind,rabie2005robust,Liu2008} still assume that the noise in real images are Gaussian distributed. They first estimate the noise level and remove the noise subsequently. However, Gaussian assumption is too simple to model the noise in comprehensively real images. Most recent works assume that the noise in real noisy images are Mixture of Gaussian distributed \cite{almapg,noiseclinic,Zhu_2016_CVPR} or tensor Gaussian cross multiple channels \cite{crosschannel2016}. 

Despite the success of these methods, they have some limitations. First of all, all these methods perform real image denoising in the framework of conjunction of a thorough noise estimation method followed by an adapted denoising method \cite{noiseclinic}. However, the real camera noise is far from being Gaussian, signal dependent, and hard to estimate \cite{crosschannel2016}. For instance, the in-camera imaging pipeline in \cite{crosschannel2016} include image demosaicing, white balance and color space transform, gamut mapping, tone mapping, and JPEG comression. During these procedures, at least five different types of noise are added, including fixed pattern noise, dark current noise, short noise, amlifier noise and quantization noise \cite{tsin2001statistical}. Although the Mixture of Gaussian model can be used to approximate any continuous distribution \cite{}, its parameters are often time consuming to be fit on the test noisy image no matter the EM \cite{em} algorithm or variational inference \cite{Bishop} is used. Hence, it is hard to predict Secondly, the methods of Portilla \cite{fullyblind}, Rabie et al. \cite{rabie2005robust}, and Liu et al. \cite{Liu2008} assumes that the noise is of Gaussian distribution. This is not true in real world noisy images.  As suggested in \cite{Liu2008,noiseclinic}, Gaussian noise may be inflexible for more complex noise in real images. On the other hand, the method \cite{almapg} needs tune specific parameters for different types of noise. This makes the proposed method not a strictly real image denoising. Hence, it is still desirable to design an robust and effective model for real image denoising, which bases on few assumptions and can deal with unknown noise without any parameter tuning procedure. 
